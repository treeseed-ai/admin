...
interface BuildInfrastructureProjectionInput {
	store: any;
	principal?: any;
	team?: any | null;
	projects?: any[];
...
interface InfrastructureBundle {
	project: any;
	summary: any | null;
	details: any | null;
	agents: any | null;
...
async function loadProjectBundle(input: BuildInfrastructureProjectionInput, project: any): Promise<InfrastructureBundle> {
	const store = input.store;
...
function projectItem(project: any): InfrastructureItem {
	return {
...
function repositoryItems(bundle: InfrastructureBundle): InfrastructureItem[] {
...
function deploymentItems(bundle: InfrastructureBundle): InfrastructureItem[] {
	const deployments = [
...
function providerItem(provider: any): InfrastructureItem {
	return {
...
function capacityOperationItems(bundle: InfrastructureBundle): InfrastructureItem[] {
...
function projectResourceItem(bundle: InfrastructureBundle, resource: any): InfrastructureItem {
...
function productItem(product: any): InfrastructureItem {
...
function policyItems(bundle: InfrastructureBundle): InfrastructureItem[] {
	return [
...
function seedItems(seedState: any): InfrastructureItem[] {
	if (!seedState) return [];
	return [
		{
			id: 'seed-plan',
...
function diagnosticsFromCapacity(teamCapacitySummary: any, bundles: InfrastructureBundle[]): InfrastructureItem[] {
...
function diagnosticsFromHosts(hosts: InfrastructureItem[]): InfrastructureItem[] {
...
function diagnosticsFromSeeds(seedState: any): InfrastructureItem[] {
...
function auditDiagnosticItem(event: any): InfrastructureItem {
...
function emptyProjection(): InfrastructureProjection {
	return {
		metrics: [],
		projects: [],
		repositories: [],
...
async function call(store: any, method: string, ...args: any[]) {
...
function compareItemDesc(left: InfrastructureItem, right: InfrastructureItem) {
...
function projectName(bundle: InfrastructureBundle) {
	return compact(bundle.project?.name, compact(bundle.project?.slug, 'Project'));
}

function seedSummary(plan: any) {
	const summary = plan?.summary;
	if (!summary) return 'Seed plan available for operator review.';
...
function titleFromEvent(value: unknown) {
