...
export function deleteAccountEmailMethod(this: ApiClientFacade, emailId: string) { return this.invoke(CONTROL_PLANE_OPERATIONS.accounts.removeEmail, { path: { emailId }, query: {}, body: {} }).then((result: any) => result.items ?? result) as Promise<AccountEmailAddress[]>; }
