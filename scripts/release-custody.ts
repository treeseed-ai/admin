...
if (process.argv[2] === 'seal') {
	...
	const imageDigest = process.env.TREESEED_ADMIN_DIGEST;
	if (!/^sha256:[a-f0-9]{64}$/u.test(imageDigest ?? '')) throw new Error('Exact Admin OCI manifest digest is required.');
	const artifacts: Array<{ id: string; kind: 'oci-image' | 'npm-package' | 'archive' | 'sbom' | 'component-manifest' | 'compose'; identity: string; digest: `sha256:${string}`; mediaType: string; size?: number }> = [{ id: 'admin-image', kind: 'oci-image', identity: `treeseed/admin@${imageDigest}`, digest: imageDigest as `sha256:${string}`, mediaType: 'application/vnd.oci.image.index.v1+json' }];
